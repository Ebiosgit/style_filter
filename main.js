...
[출력 형식]
"업로드한 원본 사진의 고유한 밝기, 색감, 조명 분위기와 [실사 피사체]는 수정 없이 완벽하게 원본 그대로 유지합니다. 이 사진 위에 손으로 그린 듯한 [드로잉 스타일]의 라인 드로잉과 낙서 요소들을 오버레이 합니다. [배경 요소들]을 검은색 외곽선 스케치와 색연필 질감으로 표현하고, [하늘/여백]에는 [추천 낙서 요소]들을 추가하고, [피사체]에도 [추천 낙서 요소들]을 추가해 주세요. 실사와 드로잉이 조화를 이루는 감성적인 포스터 스타일로 생성해 주세요."
